 ...
 * old path.
 *
 * The cases below assert the **post-fix contract** directly. They
 * fail today because the contract isn't satisfied (no reflect
 * arrives within budget). The SSH integration job will report red on
 * `next` until the production fix lands and the assertions turn
 * green. SSH integration is a non-required check on `next`, so the
 * framework PR can merge even while these are failing.
 *
 * The failure message identifies the specific issue: a timeout
 * shape ("FakeFileExplorer: no <event> ...") means #341 is
 * unaddressed; any other shape (TypeError, ECONNRESET) is an infra
...
describe('Layer 1 — writer self-reflect (SFTP transport)', () => {
  ...
  it('write — adapter.write fires vault.trigger("create") on writer (#341)', async () => {
  ...
  it('modify — adapter.write (overwrite) fires vault.trigger("modify") (#341)', async () => {
  ...
  it('rename — adapter.rename fires vault.trigger("rename") on writer (#341)', async () => {
  ...
  it('delete — adapter.remove fires vault.trigger("delete") on writer (#341)', async () => {
  ...
  //
  // Companion snapshot view of #341: after rename, vault.fileMap should
  // contain the new path and not the old. Both assertions fail today;
  // they will turn green when the production fix lands.

  it('post-rename — writer.vault.fileMap reflects the new path, not the old (#341)', async () => {
